 ...
 * fetch any records and render the user element in the navigation bar, that
...
router.post('/login/password', async (req, res, next) => {
  passport.authenticate('local', (err, user, info) => {
    ...
    req.logIn(user, (err) => {
      if (err) {
        req.session.message = 'Failed to log in user.';
        return next(err);      
      }
      if (user.RoleID === req.roles.ADMIN) {
        req.session.message = 'Welcome, Admin!';
        return res.redirect('/admin');
      } else if (user.RoleID === req.roles.CLIENT) {
        req.session.message = 'Welcome, Client!';
